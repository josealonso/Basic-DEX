...
export const swapTokens = async (
    ...
    let tx;
    // If Eth is selected call the `ethToCompluToken` function else
    // call the `compluTokenToEth` function from the contract
    ...
    if (ethSelected) {
        tx = await exchangeContract.ethToCompluToken(
            tokenToBeReceivedAfterSwap,
            {
                value: swapAmountWei,
            }
        ...
        // User has to approve `swapAmountWei` for the contract because `Complu` token
        // is an ERC20
        ...
        await tx.wait();
        // call compluTokenToEth function which would take in `swapAmountWei` of `Complu` tokens and would
        // send back `tokenToBeReceivedAfterSwap` amount of `Eth` to the user
        tx = await exchangeContract.compluTokenToEth(
            swapAmountWei,
            tokenToBeReceivedAfterSwap
        );
    }
